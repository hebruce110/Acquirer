...
//typedef for ChinaPNR json post fommat
//since post json format is not the standard way, so we have to compromise to revise the inner code
typedef enum _ASIPostBodyFormat{
    ASIURLEncodedPostGeneralFormat = 1,
    ASIURLEncodedPostJSONFormat
} ASIPostBodyFormat;
...
@interface ASIFormDataRequest : ASIHTTPRequest <NSCopying> {
    ...
    ASIPostBodyFormat postBodyFormat;
...
@property (assign) ASIPostBodyFormat postBodyFormat;
