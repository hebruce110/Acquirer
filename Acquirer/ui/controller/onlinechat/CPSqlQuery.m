...
{																\
NSLog(@"Failed executing: %s", ""#x);			\
NSLog(@"   Result code: %d", result);			\
NSLog(@"   %s", sqlite3_errmsg(self.dbHandle));	\
NSLog(@"   in query: %@", self.queryString);		\
...
{	self.lastStepResult = sqlite3_step(self.compiledStatement);
	...
	{
		NSLog(@"Failed stepping query");
		NSLog(@"   Result code: %d", self.lastStepResult);
		NSLog(@"   %s", sqlite3_errmsg(self.dbHandle));
		NSLog(@"   in query: %@", self.queryString);
        [[NSException exceptionWithName:@"SQL Error"
...
-(int)parameterIndex:(NSString*)parameterName
{
    ...
    if(index == 0)
    {
        NSLog(@"Invalid named parameter :%@ in query: %@", parameterName, self.queryString);
...
-(int)columnIndex:(NSString*)columnName
{
	...
	NSLog(@"Invalid column name %@ in result-set for: %@", columnName, self.queryString);
    [[NSException exceptionWithName:@"SQL Error"
